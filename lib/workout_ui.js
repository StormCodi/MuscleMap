...
export function createWorkoutUI({
  ...
  // NEW: mark used exercises in dropdown so they “disappear / are marked used”
  function updateExerciseSelectUsage() {
    if (!exerciseSelect) return;

    // only applies in LIVE workout mode
    if (!activeWorkout || viewingWorkoutId) return;

    const used = new Set((viewingSets || []).map((s) => String(s.exercise_id)));

    for (const opt of [...exerciseSelect.querySelectorAll("option")]) {
      const v = String(opt.value || "");
      if (!v) continue;

      if (!opt.dataset.origText) opt.dataset.origText = opt.textContent || v;

      if (used.has(v)) {
        opt.disabled = true;
        // mark once (restore from origText first to avoid stacking)
    ...
    // if current selection got disabled (exercise already used), move selection to first enabled option if possible
    const cur = String(exerciseSelect.value || "");
    const curOpt = cur ? exerciseSelect.querySelector(`option[value="${CSS.escape(cur)}"]`) : null;
    if (curOpt && curOpt.disabled) {
      const firstEnabled = [...exerciseSelect.querySelectorAll("option")].find((o) => String(o.value || "") && !o.disabled);
      if (firstEnabled) exerciseSelect.value = firstEnabled.value;
    }
  }

  function isExerciseUsedInLiveWorkout(exId) {
    if (!activeWorkout || viewingWorkoutId) return false;
    const used = new Set((viewingSets || []).map((s) => String(s.exercise_id)));
    return used.has(String(exId));
  }
  ...
  const editor = createWorkoutEditor({
    ...
    onSetsChanged: (completedSetsOnly) => {
      onEditorSetsChanged?.(completedSetsOnly);
      // used-exercise UI depends on *all* sets (not only completed)
      updateExerciseSelectUsage();
    },
  ...
  async function viewWorkout(workoutId) {
    ...
    // past edit: keep dropdown as-is (controls disabled anyway)
  }
  ...
  async function addExerciseAsOneSet() {
    ...
    // NEW: prevent adding same exercise twice in the same workout
    if (isExerciseUsedInLiveWorkout(exId)) {
